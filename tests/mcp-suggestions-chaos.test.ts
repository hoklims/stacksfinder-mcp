...
import type {
  DetectedStack,
  MCPRecommendation,
} from "../src/tools/project-kit/types.js";
import {
  MCP_REGISTRY,
  getMCPRegistryEntry,
  getRequiredEnvVars,
...
describe("TECH_MCP_MAPPINGS - Chaos Tests", () => {
  ...
  it("should have valid slugs (lowercase, no spaces)", () => {
    for (const mapping of TECH_MCP_MAPPINGS) {
      // Allow simple slugs (my-mcp) and scoped npm packages (@scope/package-name)
      expect(mapping.mcpSlug).toMatch(/^(@[a-z0-9-]+\/)?[a-z0-9-]+$/);
...
describe("matchMCPsForTechnologies - Chaos Tests", () => {
  it("should handle empty technology array", () => {
    ...
    const universalCount = result.filter(
      (r) => r.matchedTech === "universal",
    ).length;
  ...
  it("should be case-insensitive for matching", () => {
    ...
    const results = CHAOS_INPUTS.casing.map((c) =>
      matchMCPsForTechnologies([c]),
    );

    // Each should find supabase-mcp (if properly case-insensitive)
    const supabaseMatches = results.map((r) =>
      r.find((m) => m.slug === "supabase-mcp"),
    );
  ...
  it("should handle postgres variations", () => {
    ...
    const results = CHAOS_INPUTS.similar.map((s) =>
      matchMCPsForTechnologies([s]),
    );
  ...
  it("should be deterministic - same input produces same output", () => {
    ...
    const results = Array.from({ length: 5 }, () =>
      matchMCPsForTechnologies(techs),
    );
...
describe("generateInstallConfig - Chaos Tests", () => {
  ...
  it("should handle many MCPs without performance issues", () => {
    ...
    expect(
      Object.keys(config.claudeDesktop.mcpServers as Record<string, unknown>)
        .length,
    ).toBe(50);
  });
});
...
describe("executeCheckCompatibility - Chaos Tests", () => {
  ...
  it("should handle unknown MCPs gracefully", () => {
    const result = executeCheckCompatibility({
      mcps: ["unknown-mcp-1", "unknown-mcp-2"],
    });
  ...
  it("should handle duplicate MCPs", () => {
    const result = executeCheckCompatibility({
      mcps: ["neon-mcp", "neon-mcp", "neon-mcp"],
    });
